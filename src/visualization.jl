...
#       "SFV": only include stocks, flows, variables (including sum variables)
#
#       Note: The `type` parameter is only used for schema C.
#             If C0 is used, all components are always shown.
function GraphF(
    p::AbstractStockAndFlow0;
    make_stock::Function=def_stock,
    make_auxiliaryV::Function=def_auxiliaryVF,
    make_sumV::Function=def_sumV,
    make_cloud::Function=def_cloud,
    make_flow_V::Function=def_flow_V,
    make_flow_noneV::Function=def_flow_noneV,
    make_link::Function=def_link,
    make_parameter::Function=def_parameter,
    schema::String="C",
    type::String="SFVL",
    rd::String="LR"
)
    ...
    if schema == "C"
        boundInFlowNodes = [  # id is e.g., "fs_1u"
            Node(make_cloud(edgeInFlows[s],"u")...)
            for s in 1:length(edgeInFlows)
        ]
        boundOutFlowNodes = [ # id is e.g., "fs_1d"
            Node(make_cloud(edgeOutFlows[s],"d")...)
            for s in 1:length(edgeOutFlows)
        ]
    end
    ...
    g = Digraph("G", stmts; graph_attrs=graph_attrs)
    return g
end
...
"""
function GraphRB(
    c::Union{CausalLoopPM, CausalLoopPol};
    cycle_color=:yellow,
    edge_label_color=:lightblue
)
    nodes = [
        Node(
            "n$n",
    ...
    for k in 1:nedges(c)
        ...
        new_node_index = length(nodes) + 1

        # Graphviz doesn't allow us to have an edge from an edge, so we put a
        # node in the middle of each edge and use that instead.
